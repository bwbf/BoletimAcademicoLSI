...
public class AlunoDAO extends ConnectionFactory{
	...
	public ArrayList<DataProvas> listarDatas(Aluno a) {
		...
		try{
			...
			while(rs.next()){
				...
				data.setData_prova((rs.getString("data_avaliacao")));
				datas.add(data);
			}
		}catch (Exception e) {
	...
	public boolean inserirProva(DataProvas a) throws SQLException {
		...
		PreparedStatement pstst = null;
		connection = criarConexao();
        String sql = "INSERT INTO anotacoes(id_aluno,id_disciplina,data_avaliacao) VALUES(?,?,?)";
        Boolean retorno = false;
		pstst = connection.prepareStatement(sql);
		Date data = new Date(System.currentTimeMillis());
        try {
        	pstst.setInt(1,a.getId_aluno());
        	pstst.setInt(2, a.getId_disciplina());
        	pstst.setString(3, a.getData_prova());
	...
	public Disciplina getDisciplinaPeloNome(String nome) {
		Connection connection = null;
		connection = criarConexao();
		PreparedStatement pstst = null;
		ResultSet rs = null;
		...
		disciplina = new Disciplina();
		try{
			pstst = connection.prepareStatement("select * from disciplinas where nome=?");
			pstst.setString(1, nome);
			rs = pstst.executeQuery();
			disciplina = gerarDisciplina(disciplina, rs);
			}catch (Exception e) {
				// TODO: handle exception
